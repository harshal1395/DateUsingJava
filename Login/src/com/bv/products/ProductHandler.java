...
public class ProductHandler {
	...
	public static int addProducts(Products products) {
		int rowupdated = 0;
		String sql = "insert into products (productname,manufacturername,price,manufacturedate)" + "values(?,?,?,?)";
		...
		try {
			...
			preparedStatement.setDate(4, (Date) products.getManufactureDate());
	...
	public static int updateProducts(Products products) {
		int rowupdated = 0;
		String sql = "update products set productname=?,manufacturername=?,price=?,manufacturedate=? where id=?";
		Connection connection = DBConnection.getConnection();
	...
	public static List<Products> getAllProducts() {
		...
		try {
			...
			while (rs.next()) {
				...
				products.setManufactureDate(rs.getDate("MANUFACTUREDATE"));
				list.add(products);
			}
